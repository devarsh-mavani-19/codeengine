...
class Job {
  constructor({ runtime, file, args, stdin, timeout, memory_limit }) {
    ...
    uid %= config.runner_uid_max.default - config.runner_uid_min.default + 1;
    gid %= config.runner_gid_max.default - config.runner_gid_min.default + 1;
        ...
        promises.push(
          new Promise((resolve, reject) => {
            ...
            const kill_timeout =
              (timeout_compile >= 0 &&
                setTimeout(async (_) => {
                  console.log(`Timeout exceeded timeout=${timeout_compile}`);
    ...
    while (processes.length > 0) {
      ...
      processes = proc_ids.map((proc_id) => {
        ...
        try {
          ...
          const uid_line = proc_lines.find((line) => line.startsWith("Uid:"));
      ...
      for (const proc of processes) {
        ...
        try {
          ...
          console.log(`Got error while SIGSTOPing process ${proc}:`, e);
        }
      }
      ...
      for (const proc of processes) {
        ...
        try {
          ...
          console.log(`Got error while SIGKILLing process ${proc}:`, e);
        }
